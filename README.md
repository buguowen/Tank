...
在版本迭代中针对联机体验的深度优化与 Bug 根治记录：

| 模块 | 修复问题 | 修复原理 |
| :--- | :--- | :--- |
| **客户端 ByteArray** | `MsgSyncTank` 解析失败，抛出 JSON 追加多余字符报错 | 修正 `MoveBytes` 方法的索引更新顺序：先捕获当前 `Length` 到局部变量，再修改指针，确保 Compact 操作时 `writeIdx` 正确收缩，阻止了未收全的数据包强行反序列化脏数据。 |
| **服务端网络层** | 连接非正常断开时，抛出 `ObjectDisposedException` 连接报错 | 在 `OnReceiveData` / `ReceiveCallBack` 中增设健全的 Socket 存活状态与 Disposed 标识检测。 |
| **血量同步模块** | 受击扣血多端不一致，死后胜负判定不匹配 | 服务端将 `MsgHit` 的 `id` / `hp` 修正为受击者。客户端修正 `OnMsgHit` 对坦克实例的寻址逻辑，并以服务端权威 HP 为标准校准。 |
| **炮台旋转同步** | 玩家旋转车身后，另一端看到该炮台朝向错乱 | 将 `CtrlTank.cs` 发送的炮台偏转参数由世界角度 `eulerAngles.y` 修改为局部角度 `localEulerAngles.y`。 |
